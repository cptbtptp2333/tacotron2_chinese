...
import numpy as np
import torch
from config import sampling_rate, ref_wav, text_cleaners
...
def parse_args():
    ...
    parser.add_argument('--max_norm', default=2, type=float, help='Gradient norm threshold to clip')
    # minibatch
...
def test(model, step_num, loss, get_mel):
    ...
    sequence = np.array(text_to_sequence_mandarin(text, text_cleaners))[None, :]
