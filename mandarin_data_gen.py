...
# @Version      : 1.0
# @Author       : 任洁
# @Date         : 2022-04-12 15:27:19
# @LastEditors  : 任洁
# @LastEditTime : 2022-05-01 16:05:58
# @FilePath     : /Desktop/GST-Tacotron/mandarin_data_gen.py
...
from text import text_to_sequence, text_to_sequence_mandarin
...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, split, hparams):
        self.data_dir = data_dir
        data_path = os.path.join(data_dir,'BZNSYP_{}_data.csv'.format(split))
    ...
    def get_mel_text_pair(self, sample):
        ...
        audiopath, text = sample[0], sample[1]
    ...
    def get_text(self, text):
        text = text_to_sequence_mandarin(text, text_cleaners)
...
if __name__ == '__main__':
    ...
    train_dataset = TextMelLoader(data_dir, 'train', config)
    print('len(train_dataset): ' + str(len(train_dataset)))

    dev_dataset = TextMelLoader(data_dir, 'val', config)
    print('len(dev_dataset): ' + str(len(dev_dataset)))

    text, mel = train_dataset[0]
    print('text: ' + str(text))
    text = sequence_to_text_mandarin(text.numpy().tolist())
    print('text: ' + str(text))
