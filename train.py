...
def train_net(args):
    ...
    # Custom dataloaders
    train_dataset = TextMelLoader(config.csv_dir, 'train', config)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, collate_fn=collate_fn,
                                               ...
                                               drop_last=True)
    valid_dataset = TextMelLoader(config.csv_dir, 'val', config)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        sf.write('audios/output-{}.wav'.format(epoch), audio, config.sampling_rate, 'PCM_24')
