...
class SER(nn.Module):
    ...
    def forward(self, inputs):
        ...
        style_embed = self.stl(enc_out)  # TODO: 加不加stl层
        output = self.classify(style_embed)
...
class ClassifyLayer(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        linear_layer_num = len(hp.classify_filters) - 1
        linear = [nn.Linear(hp.classify_filters[i], hp.classify_filters[i + 1]) for i in range(linear_layer_num)]
        self.linear_layer = nn.ModuleList(linear)
        self.dropout_layer = nn.ModuleList([nn.Dropout(p = hp.dropout_p) for _ in range(linear_layer_num)])

    def forward(self, inputs):
        for linear, dropout in zip(self.linear_layer, self.dropout_layer):
            inputs = linear(inputs)
            # inputs = dropout(inputs)  # TODO: 是否dropout
        return inputs